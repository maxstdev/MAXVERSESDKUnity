...
namespace maxstAR
{
    ...
    public class AbstractCameraBackgroundBehaviour : MonoBehaviour
    {
		...
		private IntPtr beforeTexturePtr;
		...
		private void CreateCameraTexture(IntPtr[] textureId)
		{
			if(textureId[0] == IntPtr.Zero)
            {
				return;
            }
            ...
            {
				if (beforeTexturePtr != textureId[0] && rgbaTexture != null)
				{
					rgbaTexture.UpdateExternalTexture(textureId[0]);
					beforeTexturePtr = textureId[0];
					return;
				}
				...
				rgbaTexture = Texture2D.CreateExternalTexture(0, 0, TextureFormat.RGBA32, false, false, textureId[0]);
				rgbaTexture.filterMode = FilterMode.Bilinear;
				rgbaTexture.wrapMode = TextureWrapMode.Repeat;

				if (Application.platform == RuntimePlatform.Android)
				{
					GetComponent<Renderer>().material = new Material(Shader.Find(ARShaderName));
				}

				if (Application.platform == RuntimePlatform.OSXEditor || Application.platform == RuntimePlatform.OSXPlayer)
				{
					GetComponent<Renderer>().material = new Material(Shader.Find(BGRAShaderName));
				}
				...
				Material backgroudMaterial = GetComponent<Renderer>().material;
				backgroudMaterial.SetTexture("_MainTex", rgbaTexture);
			}
			else if(textureId.Length == 2)
            {
				if(beforeTexturePtr != textureId[0] && yTexture != null)
                {
					yTexture.UpdateExternalTexture(textureId[0]);
					uvTexture.UpdateExternalTexture(textureId[1]);
					beforeTexturePtr = textureId[0];
				}
				...
				yTexture = Texture2D.CreateExternalTexture(0, 0, TextureFormat.R8, false, false, textureId[0]);
				yTexture.filterMode = FilterMode.Bilinear;
				yTexture.wrapMode = TextureWrapMode.Repeat;

				uvTexture = Texture2D.CreateExternalTexture(0, 0, TextureFormat.RG16, false, false, textureId[1]);
				uvTexture.filterMode = FilterMode.Bilinear;
				uvTexture.wrapMode = TextureWrapMode.Repeat;
		...
		public void UpdateCameraBackgroundImage(TrackedImage image)
		{
			if (!keepRendering)
			{
				return;
		...
		public void UpdateCameraBackgroundImage(IntPtr[] textureIds)
		{
			if (!keepRendering)
			{
				return;
            ...
            if (!textureCreateDone)
            {
				if(beforeTexturePtr != textureIds[0])
                {
					CreateCameraTexture(textureIds);
				}
            }
        ...
        {
            float[] planeInfo = CameraDevice.GetInstance().GetBackgroundPlaneGeometry();
